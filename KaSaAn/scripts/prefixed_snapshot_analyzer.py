...
def main(args=None):
    """Get cumulative and mean distribution of complex sizes, plus distribution of number of species, based on
    snapshots sharing a common prefix, like "t_one_snap_7.ka" having the prefix `t_one_`. Snapshots must contain the
    word `snap` and end in `.ka`. Files will be produced in the same directory as the snapshots are. They will be
    prefixed accordingly, e.g. `[prefix]distribution_cumulative.csv`"""
    if args is None:
        args = sys.argv[1:]
