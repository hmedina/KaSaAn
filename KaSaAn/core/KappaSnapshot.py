...
class KappaSnapshot(KappaEntity):
    ...
    def to_cytoscape_cx(self):
        ...
        snap_net = self.to_networkx()
        # construct "node" and "nodeAttributes" structures
        cx_nodes = []
        cx_node_attributes = []
        for node_ident, node_data in snap_net.nodes.data():
        ...
        edge_ident = 0 + snap_net.number_of_nodes()
        cx_edges = []
        cx_edge_attributes = []
        for e_source, e_target, e_data in snap_net.edges.data():
            # find the port names
            port_names = []
            e_source_kappa = snap_net.nodes.data()[e_source]['kappa']
            e_target_kappa = snap_net.nodes.data()[e_target]['kappa']
            for this_agent in [e_source_kappa, e_target_kappa]:
                for this_site in this_agent.get_agent_signature():
                    if type(this_site) is KappaPort:
        ...
        # build network attribute structure
